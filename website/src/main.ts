...
function linkCards(links: readonly { title: string; body: string; href: string; meta: string }[]): string {
  return links
    .map(
      (item) => `
      ...
      <main>
        <section class="hero">
          ...
          <div class="hero-panel" aria-label="${config.hero.panel.title}">
            ...
            <p>${config.hero.panel.body}</p>
            <div class="chips">${config.hero.panel.chips.map((chip) => `<span>${chip}</span>`).join('')}</div>
          </div>
        </section>
        ...
        <section class="section" id="capabilities">
          <div class="section-head">
            ...
            <h2>${config.sections.capabilities.title}</h2>
          </div>
        ...
        <section class="section muted" id="workflow">
          <div class="section-head">
            ...
            <h2>${config.sections.workflow.title}</h2>
          </div>
          <div class="step-grid">
        ...
        <section class="section install" id="install">
          <div>
            ...
            <h2>${config.sections.install.title}</h2>
            <p>${config.sections.install.body}</p>
          </div>
        ...
        <section class="section links" id="links">
          <div class="section-head">
            ...
            <h2>${config.sections.links.title}</h2>
          </div>
          <div class="link-grid">
        ...
        <section class="section support" id="support">
          <div class="section-head">
            ...
            <h2>${config.sections.support.title}</h2>
            <p>${config.sections.support.body}</p>
          </div>
